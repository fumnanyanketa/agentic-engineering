...
.progress-panel.compact .cp-pct{font-family:Poppins;font-weight:700;font-size:1.15rem;white-space:nowrap;color:#e8eef6}
...
function render(){
  ...
  const s=CS.stats();
  set("s-count", s.done+"/"+s.total);
  set("s-pct", s.pct+"%");
  const bar=document.getElementById("bar"); if(bar) bar.style.width=s.pct+"%";
}
