...
> **Unit 9 of 12:** Build a graded eval suite so you can measure quality instead of guessing it, then let an agent run unsupervised because the evals gate its work
...
- Unit 3 (prompting and context): you authored Scout's system prompt and a tiny eval harness there. This unit grows that into a real graded suite.
...
**3. Calibrate against human labels before you trust it at scale.** Model judges have known biases, but the picture has sharpened. On current frontier judges under a single pairwise rubric, **verbosity bias** (preferring longer answers even when worse) has largely been mitigated; the one that persists is **position bias** (preferring whichever answer was shown first). Worse, there is a documented *consistency-versus-validity paradox*: the most reproducible judges can be among the least valid, so a judge being *consistent* is not proof it is *right*. The durable fix has not changed: have a person label a handful of cases by hand, then check that the judge agrees with that human before you rely on it. A judge can hallucinate just like any other model. (As a rough anchor, a well-built LLM-as-judge reaches roughly 85 percent agreement with human labels, which is why it is now the default reference-free method at scale, and also why calibration still matters.)

> 💡 **Three names worth carrying.** Two newer techniques raise judge reliability: a **panel (or jury) of judges** (several diverse judges vote, which cancels out individual quirks), and **"do before you judge"** (the judge attempts the task itself first, then evaluates, which grounds its standard in the actual difficulty). And the discipline this whole unit teaches now has a name, **eval-driven development**: treat evals as the thing you build first and regress against, the same way tests anchor good software.
...
> 💡 **Where this goes at production scale (a pointer).** What you built here is a small offline suite, exactly right for learning and for a tight inner loop. In production it grows in three directions. First, *size*: from a handful of cases to hundreds or thousands, drawn from real failures as they happen, so the suite mirrors reality. Second, *online evaluation*: instead of only running fixed cases offline, you sample real live traffic, score a slice of it (with an LLM judge or human review), and watch the score over time, so regressions surface in production, not just in your test file. Third, *gating*: the suite runs automatically on every change and blocks a deploy when the score drops. The discipline is identical to what you just practised; only the scale and the plumbing grow. Verify specifics against your eval framework's current docs.
...
- **Back to Unit 3 (Scout's prompt):** the tiny harness you wrote there grows up here into Warden's real graded suite.
- **Next, Unit 10 (and beyond):** with the gate in place, Atlas can run agents unsupervised and on a schedule, because nothing ships past Warden without passing. Later units wire these evals into the self-improving prompt loop, so Scout's instructions optimize themselves against the score.
