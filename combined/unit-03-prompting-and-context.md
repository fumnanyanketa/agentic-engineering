...
> **Unit 3 of 12:** Learn to talk to a model so it does what you want, and learn to give it the right information in the smallest high-signal form, then write your first versioned system prompt for an AtlasOS agent
...
- **Next, Unit 4 (Picking the right model and the thinking lever):** now that you can write a prompt, you learn to choose which model runs it and how hard it should think, judged with real tests, "cost per successful outcome" rather than cost per token. Scout's prompt is what you will route.
