...
> **Unit 10 of 12:** See what your agent actually did (logs, traces, cost), defend it from the attacks that get systems pulled, harden it so it survives failures, then deploy it to a real cloud so it keeps running when you close your laptop
...
```text
git add -A
git commit -m "Unit 10: add ops (tracing, guardrail, human gate, resilience) and deploy Scout to cloud"
git push
```
...
- **Earlier, Unit 8 (Multi-agent orchestration):** the multi-agent system you orchestrated there is exactly what you now make observable, safe, and deployable, each sub-agent becomes a set of spans in your trace.
...
- **Next, Unit 11:** with Scout deployed, observable, and guarded, you turn from building one production agent to running the fleet, the operating model for a founder plus agents.
